...
export default function CenterOppositionTestsPage({
  params,
}: {
  params: Promise<{ slug: string; oppositionSlug: string }>;
}) {
  const { slug, oppositionSlug } = use(params);
  const oposicion = useMemo(() => getOposicionBySlug(oppositionSlug), [oppositionSlug]);
  ...
  if (!oposicion) {
    return (
      <AuthGuard
        resource="tests"
        action="read"
        fallbackPath={`/centro/${slug}/oposiciones`}
      >
        <Card className="border-slate-200">
          <CardContent className="py-10 text-center text-sm text-slate-500">
            Oposición no disponible para el modo de tests.
          </CardContent>
        </Card>
      </AuthGuard>
    );
  ...
  const completeTest = () => {
    ...
    const topicLabel =
      selectedTopic && selectedTopic !== 'general' ? (topicMap[selectedTopic] ?? selectedTopic) : 'General';
  ...
  if (testComplete) {
    ...
    return (
      <AuthGuard resource="tests" action="read" fallbackPath={basePath}>
        <Card className="border-slate-200">
          ...
          <CardContent className="space-y-6">
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              ...
              <Link href={`${basePath}/flashcards`}>
                <Button className="w-full bg-amber-600 text-white hover:bg-amber-500">
                  Ir a flashcards
                </Button>
              </Link>
              <Link href={`${basePath}/dashboard`}>
                <Button className="w-full bg-[#1B3A5C] text-white hover:bg-[#16314d]">
                  Ver dashboard
                </Button>
              </Link>
            </div>
    ...
    return (
      <AuthGuard resource="tests" action="read" fallbackPath={basePath}>
        <div className="space-y-6">
          <div>
            <h2 className="text-xl font-semibold text-[#1B3A5C]">Tests por bloque</h2>
            <p className="text-sm text-slate-500">
              Cada test genera 10 preguntas adaptadas a {oposicion.name}.
            </p>
          </div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <Card className="cursor-pointer border-2 border-[#1B3A5C]" onClick={() => startTest('general')}>
              <CardHeader>
                ...
                <CardDescription>
                  Mezcla de todos los temas para entrenar visión global.
                </CardDescription>
              </CardHeader>
            </Card>
      ...
      <AuthGuard resource="tests" action="read" fallbackPath={basePath}>
        <Card className="border-slate-200">
          <CardHeader>
            ...
            <CardDescription>
              No se pudieron cargar preguntas para este bloque. Prueba con otro tema.
            </CardDescription>
          </CardHeader>
          <CardContent>
